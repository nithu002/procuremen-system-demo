    ...
    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
        ...
        <li class="nav-item  menu-open">
            ...
            <ul class="nav nav-treeview">
                ...
                <li class="nav-item">
                    <a href={{ url('visor') }} class="nav-link active">
...
@section('sub-heading')
   ...
   <div class="col-sm-6">
      <h1 class="m-0">Supervisor Details</h1>
   </div>
   <div class="col-sm-6">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Supervisor Creat Form</h5>
          <a data-bs-dismiss="modal"><i class="fa fa-times"></i></a>
        </div>
        <form action="{{ url('/visor') }}" method="POST" enctype="multipart/form-data">
            <div class="modal-body">
                <div class="mb-3">
                    @csrf
                    <label for="exampleInputEmail1" class="form-label">Supervisor Name</label>
                    <input type="text" name="super_name" class="form-control" required value="{{ old('super_name') }}" aria-describedby="textHelp">
                    <span class="text-danger">@error('super_name'){{ $message }} @enderror</span>
                    <div id="textHelp" class="text-black-50">Enter the supervisor name.</div>
                </div>
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Supervisor Email</label>
                    <input type="text" name="email" class="form-control" value="{{ old('email') }}" required aria-describedby="textHelp">
                    <span class="text-danger">@error('email'){{ $message }}@enderror</span>
                    <div id="textHelp" class="text-black-50">Enter the supervisor email address.</div>
                </div>
                <div class="mb-3">
 ...
 <!-- EDIT Modal -->
  <div class="modal fade" id="editModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel">Supervisor Edit Form</h5>
          <a data-bs-dismiss="modal"><i class="fa fa-times"></i></a>
        </div>
        <form action="{{ url('/visor') }}" method="POST" id="editForm" enctype="multipart/form-data">
            <div class="modal-body">
                <div class="mb-3">
                    @csrf
                    @method('PUT')
                    <label for="exampleInputEmail1" class="form-label">Supervisor Name</label>
                    <input type="text" name="super_name" id="super_name" class="form-control" required  aria-describedby="textHelp">
                    <span class="text-danger">@error('super_name'){{ $message }} @enderror</span>
                    <div id="textHelp" class="text-black-50">Enter the supervisor name.</div>
                </div>
                <div class="mb-3">
                    ...
                    <span class="text-danger">@error('email'){{ $message }}@enderror</span>
                    <div id="textHelp" class="text-black-50">Enter the vaild supervisor email address.</div>
                </div>
                <div class="mb-3">
                    <div class="form-group">
                        <label>Phone Number</label>
...
<!--DELETE MODEL-->
    <div class="modal fade" id="deleteModel" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <form action="{{ url('visor/delete') }}" method="POST"  enctype="multipart/form-data">
                <div class="modal-body">
                        <center>
        ...
        <div class="card-body">
          <table id="example1" class="table table-bordered table-hover data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td hidden>{{ $supers->id }}</td>
                    <td>{{ $supers->name }}</td>

                    <td>{{ $supers->email }} </td>

                    <td>{{ $supers->phone_no  }}</td>

                    <td>{{ $supers->address  }}</td>
                    ...
                    <td>
                        <div class="dropdown" style="float:left;">
                            ...
                            <div class="dropdown-content" style="bottom:0;">
                                ...
                                <a href={{ url("visor/mail/$supers->id ")}}><span class="fa fa-share"></span>&nbsp;Share Access</a>
                            </div>
                        </div>
                    </td>
                </tr>
...
$(document).ready(function() {
    ...
    table.on('click','.edit',function(){
        ...
        $('#super_name').val(data[1]);
        $('#email').val(data[2]);
        $('#phone_no').val(data[3]);
        $('#address').val(data[4]);

        $('#editForm').attr('action','/visor/'+data[0]);
